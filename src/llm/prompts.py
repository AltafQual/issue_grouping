"""All LLM prompt templates for the Issue Grouping system.

Every system message and user-turn template used by the QGenie / Gemini models
lives here.  Higher-level modules (cluster_namer, cluster_classifier, etc.)
import the constants they need from this module — prompt text must **never**
be inlined in business-logic code.

Naming convention
-----------------
*_SYS_MESSAGE  — system-level instructions passed to the model at the start of
                 each conversation.
*_LOG_MESSAGE  — user-turn template that injects the actual log data.
