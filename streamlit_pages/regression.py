...
with st.form("regression_form"):
    ...
    with col1:
        run_id_a = st.selectbox(
            "Select First Run ID (Base)", 
            options=tc_options, 
            index=None,
            key="run_id_a_select"
        )
    ...
    with col2:
        run_id_b = st.selectbox(
            "Select Second Run ID (Compare)", 
            options=tc_options, 
            index=None,
            key="run_id_b_select"
        )
...
if analyze_button:
    if not run_id_a or not run_id_b:
        ...
        if (run_id_a != st.session_state.last_run_ids["run_id_a"] or 
            run_id_b != st.session_state.last_run_ids["run_id_b"]):
