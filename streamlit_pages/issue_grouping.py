...
from src.pipeline.workers import tc_id_scheduler
from src.utils.excel_exporter import create_excel_with_clusters
    ...
    if current_input_type and (
        ...
        if df_to_process is None or df_to_process.empty:
            ...
            with st.spinner("Analyzing and grouping data... This may take a moment."):
                try:
                    clustered_results = asyncio.run(
                        ClusteringPipeline().run(df_to_process, mode=ExecutionMode.SEQUENTIAL)
                    )
